                    ...
                    try {
                        ...
                        if (stateIds.length > 0) {
                            ...
                            stateIds.slice(0, 5).forEach((stateId, index) => {
                                ...
                                console.log(
                                    `   ${stateId}: ${state && state.val !== undefined ? state.val : 'undefined'}`,
                                );
                            });
...
```javascript
it('should create and verify multiple states', () =>
                        ...
                        if (stateIds && stateIds.length > 0) {
                            harness.states.getStates(stateIds, (err, allStates) => {
                                ...
                                const expectedStates = [
                                    'tagesschau.0.info.connection',
                                    'tagesschau.0.articles.0.title',
                                ];
                                let foundStates = 0;
                                ...
                                if (foundStates === expectedStates.length) {
                                    ...
                                    reject(
                                        new Error(
                                            `Only ${foundStates}/${expectedStates.length} expected states were found`,
                                        ),
                                    );
                                }
                            });
...
it('Should connect to API with demo credentials', async () => {
    ...
    if (connectionState && connectionState.val === true) {
        ...
        throw new Error(
            'API Test Failed: Expected API connection to be established with demo credentials. ' +
