 ...
 * WebSocket streaming endpoints (compile, upload) use the native WebSocket API
 * which is available from Node >= 22. On Node 20 those methods throw an error.
 */
 ...
 *
 * Requires Node >= 22 for the built-in WebSocket API.
 *
...
async function streamLogs(dashboardUrl, path, spawnParams, lineReceivedCb) {
    if (typeof WebSocket === 'undefined') {
        throw new Error(
            'WebSocket streaming requires Node.js >= 22. ' +
                'Please upgrade your Node.js version to use compile/upload functionality.',
        );
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.addEventListener('message', event => {
            ...
            try {
                ...
                if (typeof console !== 'undefined' && typeof console.debug === 'function') {
                    console.debug(`Failed to parse WebSocket message from /${path}:`, error, 'raw message:', event.data);
                }
                return;
            }
        ...
        ws.addEventListener('error', err => {
            reject(new Error(`WebSocket error on /${path}: ${err.message || err}`));
        });

        ws.addEventListener('close', event => {
            if (!event.wasClean) {
                reject(new Error(`WebSocket closed unexpectedly on /${path}: code ${event.code}`));
            }
        });
    });
}
