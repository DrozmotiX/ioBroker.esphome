    ...
    try {
      ...
      if (!fs.existsSync(espHomeDir)) {
        this.adapter.log.warn(
          `ESPHome directory does not exist: ${espHomeDir}`,
        );
        return [];
      }
    ...
    try {
      ...
      if (!this.validateFilename(filename)) {
        return {
          success: false,
          error:
            "Invalid filename. Only .yaml and .yml files are allowed, and filename must not contain path separators or relative path components.",
        };
      }
