...
class Esphome extends utils.Adapter {
    ...
    try {
      ...
      if (autodiscovery) {
        if (resetTimers["autodiscovery"]) {
          resetTimers["autodiscovery"] = clearTimeout(
            resetTimers["autodiscovery"],
          );
        }
  ...
  async migrateConfig() {
    try {
      ...
      if (this.config.ESPHomeDashboardIP && !this.config.ESPHomeDashboardUrl) {
        ...
        this.log.info(
          `Migrating configuration: Setting ESPHomeDashboardUrl to ${calculatedUrl}`,
        );
        ...
        this.log.info(
          `Configuration migrated successfully. ESPHomeDashboardUrl set to: ${calculatedUrl}`,
        );
  ...
  async espHomeDashboard() {
    try {
      ...
      try {
        lastUsed = await this.getStateAsync(
          `_ESPHomeDashboard.selectedVersion`,
        );
      ...
      if (content) {
        await this.stateSetCreate(
          `_ESPHomeDashboard.versionCache`,
          "versionCache",
        ...
        await this.stateSetCreate(
          `_ESPHomeDashboard.newestVersion`,
          "newestVersion",
          content[0].name,
        );
      } else {
        ...
        let cachedVersions = await this.getStateAsync(
          `_ESPHomeDashboard.versionCache`,
        );
      ...
      if (this.config.ESPHomeDashboardEnabled) {
        this.log.info(
          `Native Integration of ESPHome Dashboard enabled, making environment ready`,
        );
        try {
          ...
          try {
            ...
            this.log.error(
              `Fatal error starting ESPHomeDashboard | ${error} | ${error.stack}`,
            );
            return;
          }
          ...
          this.log.debug(
            `espHomeDashboard_Process ${JSON.stringify(dashboardProcess)}`,
          );
          ...
          dashboardProcess.on("message", (code, signal) => {
            this.log.info(
              `[dashboardProcess MESSAGE] Exit code is: ${code} | ${signal}`,
            );
          });
  ...
  deviceDiscovery() {
    try {
      ...
      discovery = new Discovery({
        interface: this.config.discoveryListeningAddress
      ...
      discovery.on("info", (message) => {
        this.log.debug(
          `ESPHome Device found on ${message.address} | ${JSON.stringify(message)}`,
        );
        if (
          ...
          if (message.mac == null) {
            this.log.warn(
              `Discovered device with undefined mac. ignoring: ${JSON.stringify(message)}`,
            );
            return;
          }
          ...
          newlyDiscoveredClient[message.address] = {
            ...
            deviceFriendlyName: message.friendly_name
  ...
  connectDevices(host) {
    try {
      ...
      if (
        clientDetails[host] &&
        (clientDetails[host].connecting ||
          clientDetails[host].deletionRequested)
      ) {
        return;
      }
      ...
      if (!clientDetails[host].encryptionKeyUsed) {
        ...
        clientSettings.encryptionKey = this.decrypt(
          clientDetails[host].encryptionKey,
        );
      }
        ...
        try {
          ...
          if (this.messageResponse[host]) {
            this.sendTo(
              ...
              {
                result:
                  "OK - Device successfully connected, initializing configuration. Refresh table to show all known devices",
              },
      ...
      clientDetails[host].client.connection.on("message", (message) => {
        this.log.debug(
          `[ESPHome Device Message] ${host} client log ${message}`,
        );
      });
      ...
      clientDetails[host].client.on("newEntity", async (entity) => {
        ...
        try {
          ...
          clientDetails[host][entity.id] = {
            ...
            type: entity.type,
            unit:
              entity.config.unitOfMeasurement !== undefined
                ? entity.config.unitOfMeasurement || ""
                : "",
          };
          ...
          await this.extendObjectAsync(
            `${clientDetails[host].deviceName}.${entity.type}`,
            {
              type: "channel",
              common: {
                name: entity.type,
          ...
          await this.extendObjectAsync(
            `${clientDetails[host].deviceName}.${entity.type}.${entity.id}`,
            {
              type: "channel",
              common: {
          ...
          entity.on(`state`, async (state) => {
            ...
            try {
              this.log.debug(
                `[entityStateConfig] ${JSON.stringify(clientDetails[host][entity.id])}`,
              );
              ...
              switch (clientDetails[host][entity.id].type) {
                ...
                default:
                  if (!warnMessages[clientDetails[host][entity.id].type]) {
                    ...
                    warnMessages[clientDetails[host][entity.id].type] =
                      deviceDetails;
                  }
              }
            } catch (error) {
          ...
          this.log.error(
            `Connection issue for ${entity.name} ${e} | ${e.stack}`,
          );
        }
      });
      ...
      for (const stateName in clientDetails[host][entity.id].states) {
        ...
        switch (stateName) {
          case "currentTemperature":
            unit = `°C`;
            writable = false;
            clientDetails[host][entity.id].states.currentTemperature =
        ...
        if (
          ...
          if (clientDetails[host][entity.id].states.transitionLength == null) {
            ...
            try {
              ...
              if (transitionLength) {
                clientDetails[host][entity.id].states.transitionLength =
  ...
  async traverseJson(
    ...
    try {
      for (const i in jObject) {
        ...
        if (
          !!jObject[i] &&
          typeof jObject[i] === "object" &&
          jObject[i] === "[object Object]"
        ) {
          if (parent == null) {
            id = i;
            if (replaceName) {
          ...
          if (Object.keys(jObject[i]).length !== 0) {
            ...
            this.log.debug(
              `State ${id} received with empty array, ignore channel creation`,
            );
          }
        } else {
          value = jObject[i];
          ...
          if (value !== "[]") {
            this.log.debug(
              `create id ${id} with value ${value} and name ${name}`,
            );
  ...
  async stateSetCreate(
    ...
    this.log.debug(
      `Create_state called for : ${objName} with value : ${value}`,
    );
    try {
  ...
  errorHandler(codepart, error) {
    ...
    try {
      if (!disableSentry) {
        this.log.info(
          `[Error caught and send to Sentry, thank you collaborating!] error: ${errorMsg}`,
        );
        if (this.supportsFeature && this.supportsFeature("PLUGINS")) {
          const sentryInstance = this.getPluginInstance("sentry");
          if (
            sentryInstance &&
            typeof sentryInstance.getSentryObject === "function"
          ) {
    ...
    try {
      ...
      for (const device in clientDetails) {
        ...
        if (clientDetails[device].mac != null) {
          const deviceName = this.replaceAll(
            clientDetails[device].mac,
            `:`,
            ``,
          );
      ...
      switch (obj.command) {
        ...
        case "getHostIp":
          {
            ...
            const hostIP = await this.getForeignObjectAsync(
        ...
        case "_addUpdateDevice":
          ...
          if (obj.message.ip === "undefined") {
            this.sendTo(
              ...
              {
                error:
                  "To add/modify a device, please enter the IP-Address accordingly",
              },
              obj.callback,
            );
            return;
            ...
            this.sendTo(
              ...
              {
                error:
                  "Format of IP-Address is incorrect, please provide an valid IPV4 IP-Address",
              },
              obj.callback,
            );
            return;
          ...
          const initiateNDevice = async () => {
            const encryptionKeyUsed = !!(
              obj.message.encryptionKey &&
              obj.message.encryptionKey !== "undefined"
            );
            ...
            clientDetails[obj.message.ip].storeConnectDetails(
              ...
              encryptionKeyUsed
                ? this.encrypt(obj.message.encryptionKey)
                : null,
            );
          ...
          if (!clientDetails[obj.message.ip]) {
            ...
            if (resetTimers[obj.message.ip]) {
              resetTimers[obj.message.ip] = clearTimeout(
                resetTimers[obj.message.ip],
              );
            }
        ...
        case "deleteDevice":
          ...
          if (clientDetails[obj.message.ip]) {
            ...
            try {
              await this.delObjectAsync(
                clientDetails[obj.message.ip].deviceName,
                { recursive: true },
              );
            } catch {
            ...
            this.sendTo(
              ...
              {
                error:
                  "Provided IP-Address unknown, please refresh table and enter an valid IP-Address",
              },
              obj.callback,
            );
            return;
      ...
      if (state && state.ack === false) {
        ...
        if (
          ...
          if (
            ...
            clientDetails[deviceIP][device[4]].states.red =
              rgbConversion.red / 100 / 2.55;
            clientDetails[deviceIP][device[4]].states.blue =
              rgbConversion.blue / 100 / 2.55;
            clientDetails[deviceIP][device[4]].states.green =
              rgbConversion.green / 100 / 2.55;
          ...
          await clientDetails[deviceIP].client.connection.lightCommandService(
            data,
          );
        }
      }
  ...
  async resetOnlineStates() {
    try {
      ...
      const _devices = await this.getObjectViewAsync(
        "system",
        "device",
        params,
      );
  ...
  async objectCleanup(ip) {
    try {
      ...
      if (
        clientDetails[ip] &&
        (clientDetails[ip].connectionError || !clientDetails[ip].connected)
      ) {
        return;
      }
      ...
      const _channels = await this.getObjectViewAsync(
        "system",
        "channel",
        params,
      );
      ...
      for (const currDevice in _channels.rows) {
        ...
        if (
          !clientDetails[ip].adapterObjects.channels.includes(
            _channels.rows[currDevice].id,
          ) &&
          _channels.rows[currDevice].id.split(".")[2] ===
            clientDetails[ip].deviceName
        ) {
          this.log.debug(
            `[objectCleanup] Unknown Channel found, delete ${_channels.rows[currDevice].id}`,
          );
      ...
      for (const currDevice in _states.rows) {
        if (
          !this.createdStatesDetails[
            _states.rows[currDevice].id.replace(`esphome.0.`, ``)
          ] &&
          _states.rows[currDevice].id.split(".")[2] ===
            clientDetails[ip].deviceName
        ) {
          this.log.debug(
            `[objectCleanup] Unknown State found, delete ${_states.rows[currDevice].id}`,
          );
  ...
  async offlineDeviceCleanup() {
    ...
    try {
      ...
      for (const device in knownDevices) {
        // Get online value
        const online = await this.getStateAsync(
          `${knownDevices[device]._id}.info._online`,
        );
  ...
  async updateConnectionStatus(
    ...
    try {
      ...
      if (!connected || connectionError || connecting) {
        ...
        await this.stateSetCreate(
          `${clientDetails[host].deviceName}.info._online`,
          `Online state`,
          false,
        );
      } else {
