...
async function testDashboardStartup() {
  ...
  try {
    ...
    const dashboardProcess = spawn(
      "esphome",
      ["dashboard", "--port", "6052", path.join(tempDir, "esphome.0")],
      {
    ...
    return new Promise((resolve, reject) => {
            ...
            try {
              ...
              const req = http.get("http://localhost:6052/", (res) => {
                ...
                if (
                  ...
                  reject(
                    new Error(
                      `Dashboard returned unexpected status: ${res.statusCode}`,
                    ),
                  );
                }
              });
              ...
              req.on("error", (err) => {
                console.log(
                  `Dashboard not yet accessible: ${err.message}, continuing to wait...`,
                );
                ...
                setTimeout(() => {
                  const retryReq = http.get("http://localhost:6052/", (res) => {
                    if (
                      ...
                      clearTimeout(startupTimeout);
                      console.log(
                        "ESPHome Dashboard started successfully (on retry)!",
                      );
                      dashboardProcess.kill("SIGTERM");
                      setTimeout(() => {
                        try {
                          ...
                          console.warn(
                            `Cleanup warning: ${cleanupErr.message}`,
                          );
                        }
                        resolve();
                      }, 1000);
          ...
          setTimeout(() => {
            ...
            const req = http.get("http://localhost:6052/", (res) => {
              if (
                ...
                clearTimeout(startupTimeout);
                console.log(
                  "ESPHome Dashboard started successfully (from stderr)!",
                );
                dashboardProcess.kill("SIGTERM");
                setTimeout(() => {
                  try {
      ...
      dashboardProcess.on("exit", (code, signal) => {
        console.log(
          `Dashboard process exited with code ${code}, signal ${signal}`,
        );

        if (!dashboardStarted && code !== 0) {
          reject(
            new Error(`Dashboard process exited prematurely: code ${code}`),
          );
        }
      });
