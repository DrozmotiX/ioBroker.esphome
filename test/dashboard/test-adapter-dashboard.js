...
/**
 * Test adapter dashboard integration
 */
async function testAdapterDashboard() {
  ...
  try {
    ...
    console.log(
      "Attempting to create Python virtual environment via autopy...",
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      versionProcess.on("exit", (code) => {
        if (code === 0) {
          ...
          reject(
            new Error(
              `ESPHome version check failed with code ${code}: ${output}`,
            ),
          );
        }
      });
    ...
    if (
      ...
      console.log(
        "Network-related error detected - this may be expected in CI environment",
      );
