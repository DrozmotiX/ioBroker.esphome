    ...
    suite('Pillow Version Fetching', getHarness => {
        it('should fetch and cache Pillow versions during initialization', async function () {
            ...
            try {
                ...
                console.log(`✓ Successfully cached ${cachedVersions.length} Pillow versions`);
                ...
                if (newestPillowVersion) {
                    ...
                    console.log(`✓ Newest version state matches: ${newestPillowVersion.val}`);
                }

                // Verify versions are filtered (no alpha/beta/rc)
                const hasPreRelease = cachedVersions.some(v => v.includes('a') || v.includes('b') || v.includes('rc'));
